...
arrange_ggsurvplots <- function(x, print = TRUE, title = NA, ncol = 2, nrow = 1, byrow=FALSE,
                                layout_matrix = matrix(seq_len(nrow*ncol), nrow=nrow, ncol=ncol, byrow=byrow),
                                surv.plot.height = NULL, risk.table.height = NULL, ncensor.plot.height = NULL, ...)
{
  ...
  survs <- do.call(gridExtra::marrangeGrob,
                  list(grobs = survs, ncol = ncol, nrow = nrow, byrow = byrow, layout_matrix = layout_matrix, top = title))

...
plot.protein <- function(input, df, idMappingDf) {
    ...
    # pallete of colors for missense
    ...
    if (cutOff_Mut > 0 | proteinLen < max(aminoChanges$proteinPosition)) {
        cutOffComment <- paste0(sum(aminoChanges$nMutPerPos <= cutOff_Mut | aminoChanges$proteinPosition > proteinLen ),' variants not shown due to cutoff or different transcript')
    } else {
        cutOffComment <- ' '
    }
    ...
    p <- ggplot(aminoChanges, aes(x = proteinPosition, y = y_start, xend = proteinPosition, yend = y_end,
        ...
        scale_x_continuous(expand = expand_scale(mult = c(0.01, 0.05)), limits = c(0, proteinLen)) +
        scale_y_continuous(expand = expand_scale(mult = c(0, 0)), name = substitute(paste(italic(inGene), ' Mutations')),
                           breaks = c(0, maxMutations), labels = c(0, maxMutations)) +
        theme(axis.title.x=element_blank(), axis.line.x=element_blank(), axis.ticks.x=element_blank(), axis.text.x = element_blank(),
              axis.text.y = element_text(size = rel(1.4)), plot.margin=unit(c(7, 1, 0.01, 1), "mm")) +
        annotate("text", Inf, Inf, label = cutOffComment, hjust = 1, vjust = -1, color = 'red')
    ...
    # build bottom part of plot with legend
    pb <- ggplot(fakeD, aes(x=c(0,proteinLen), y=c(0,1))) +
        #theme_classic() +
        theme(axis.title.x=element_blank(), axis.title.y=element_blank(), axis.line.y=element_blank(), axis.ticks.y=element_blank(), axis.text.y = element_blank(),
              axis.text.x = element_text(size = rel(1.4)), plot.margin=unit(c(0, 1, 0, 1), "mm")) +
        scale_x_continuous(expand = expand_scale(mult = c(0.01, 0.05)), breaks = prot.breaks, labels = prot.breaks.labels) +
        scale_y_continuous(limits = c(0.15, 0.9), expand = c(0,0)) +
        annotate("rect",
                 xmin = 0, xmax = proteinLen,
                 ymin = 0.45, ymax = 0.75,  fill = "grey", alpha=.8)
    if (isLegend) {
        pb <- pb + geom_tile(aes(1:3, Missense, color = Missense, fill = factor(const, levels = c('Nonsense', 'Synonymous', 'Other')))) +
            ...
            guides(fill = guide_legend(title = element_blank(), label.theme = element_text(size = 16), order = 2),
                   color = guide_colourbar(barwidth = 1.5, barheight = 1, label = F, ticks = F, direction = "horizontal",
                                           title.position = 'right', title.vjust = 0.75, title.theme = element_text(size = 16),
                                           order = 1)) +
            theme(legend.position="bottom", legend.margin = margin(t = 0, r = 0, b = 0.1, l = 0, unit = "mm"),
                  legend.justification = 'center')
    }
    for (rowI in 1:nrow(parsedGraphics)) {
        pb <- pb + annotate("rect",
                            xmin = parsedGraphics[rowI, 'start'], xmax = parsedGraphics[rowI, 'end'],
                            ymin = 0.3, ymax = 0.9,  fill = parsedGraphics[rowI, 'colour'], alpha = .95) +
            annotate("text",
